...
get_diagnostics <- function(model, model_name) {
  ...
  diag <- as_draws_df(model) %>%
    ...
    filter(!str_detect(variable, "^lp__"))
...
post <- post %>%
  mutate(
    Section = case_when(
      ...
      str_detect(Parameter, "^sd_id_mere__") ~ "Among-mother SD",
      str_detect(Parameter, "^cor_id_mere__") ~ "Among-mother correlation",
...
::: {#fig-sone fig-cap="Relationship between pronotum length and forceps length in female, brachylabic male, and macrolabic male Forficula auricularia sensu lato. Points represent individual earwigs. Lines show fitted values from the Bayesian allometry model for each combination of diet treatment (good versus poor) and density treatment (1, 4, or 8 individuals per dish). Females and brachylabic males exhibited broadly similar scaling relationships across environments, whereas macrolabic males showed greater divergence among treatment combinations. Note that macrolabic males were uncommon under poor-diet conditions (Table S1), and estimates for these environments should therefore be interpreted cautiously."}
![](figure_S1.jpg)
:::

\newpage

::: {fig-stwo fig-cap="Relationships between pronotum length and forceps length predicted by the quadratic Bayesian allometry model for females, brachylabic males, and macrolabic males across developmental diet and density treatments. Points represent individual adults. Lines show posterior median population-level predictions, and shaded regions show 95% credible intervals. Predictions are restricted to the body-size range observed within each phenotypic group, diet, and density combination. The quadratic model indicated modest curvature in several relationships but supported the same qualitative conclusions as the primary linear model. Apparent curvature in several macrolabic male relationships was concentrated near the extremes of the observed body-size range, where estimates were influenced by relatively few observations. Macrolabic males were uncommon under poor-diet conditions, particularly at low density (n = 3), medium density (n = 18), and high density (n = 13), and these curves should therefore be interpreted cautiously."}
![](figure_S2.jpg)
:::
