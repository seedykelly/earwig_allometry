...
#### Slope plasticity
Female slopes were relatively stable across environments. The interaction between body size and diet was centred near zero (log pronotum:dietPOOR = `r myround(tidy(mod_sex)[8,5],2)`, 95% CrI: `r myround(tidy(mod_sex)[8,7],2)`–`r myround(tidy(mod_sex)[8,8],2)`; Table S1), and density interactions similarly overlapped zero. Female slopes were relatively stable across environments (@fig-four).

In contrast, males exhibited pronounced environment-dependent changes in scaling exponent (@fig-four). Under a poor diet, the male slope declined substantially, as indicated by the negative three-way interaction (log pronotum × sexmale × dietPOOR = `r myround(tidy(mod_sex)[14,5],2)`, 95% CrI: `r myround(tidy(mod_sex)[14,7],2)`–`r myround(tidy(mod_sex)[14,8],2)`; Table S1). This corresponds to a reduction from approximately `r myround(tidy(mod_sex)[2,5] + tidy(mod_sex)[7,5], 2)` under a good diet to approximately `r myround(tidy(mod_sex)[2,5] + tidy(mod_sex)[7,5]-tidy(mod_sex)[8,5]-tidy(mod_sex)[7,5], 2)` under a poor diet. Posterior probability that male slope decreased under a poor diet exceeded `r mean(beta_male_poor < beta_male_good)`, and the probability that the magnitude of change exceeded 0.1 was > `r myround(mean(abs(delta_male_diet) > 0.1),3)`. 

Density had the opposite effect, increasing male slopes at densities 4 and 8 (log pronotum × sexmale × density4 = `r myround(tidy(mod_sex)[15,5],2)`; log pronotum × sexmale × density8 = `r myround(tidy(mod_sex)[16,5],2)`; both credible intervals excluding zero). Posterior probabilities that male slope increases exceeded 0.1 were > `r mean(abs(delta_male_density4) > 0.1)`. Across treatments, male slope plasticity exceeded female slope plasticity with high probability (> `r myround(mean(abs(delta_male_diet) > abs(delta_female_diet)),3)`).
...
Diet had little effect on the intercept in brachylabic males (dietPOOR = `r myround(tidy(mod_morph)[4,5],2)`, 95% CrI: `r myround(tidy(mod_morph)[4,7],2)`–`r myround(tidy(mod_morph)[4,8],2)`; Table S2), but reduced the intercept in macrolabic males (morphmacrolabic:dietPOOR = `r myround(tidy(mod_morph)[9,5],2)`, 95% CrI: `r myround(tidy(mod_morph)[9,7],2)`–`r myround(tidy(mod_morph)[9,8],2)`; Table S2). Nutritional stress, therefore, disproportionately reduced weapon size in the exaggerated morph.

Both morphs showed some tendency toward reduced scaling exponents under a poor diet. In brachylabic males, slope decline was modest (log pronotum:dietPOOR = `r myround(tidy(mod_morph)[8,5],2)`, 95% CrI: `r myround(tidy(mod_morph)[8,7],2)`–`r myround(tidy(mod_morph)[8,8],2)`; Table S2). In macrolabic males, the additional diet-dependent slope change in macrolabic males was positive but highly uncertain (log pronotum:morphmacrolabic:dietPOOR = `r myround(tidy(mod_morph)[14,5],2)`, 95% CrI: `r myround(tidy(mod_morph)[14,7],2)`–`r myround(tidy(mod_morph)[14,8],2)`; Table S2). Posterior contrasts indicated that diet-induced slope plasticity tended to be greater in macrolabic than brachylabic males (posterior probability ≈ `r myround(mean(abs(delta_macro_diet) > abs(delta_brachy_diet)),2)`), although credible intervals were broad.
