...
Females exhibited a continuous distribution of weapon sizes across body sizes ($\bar{x}$ ± SE pronotum length = `r myround(summary.table.2[[1,6]],2)` ± `r myround(summary.table.2[[1,8]],3)` mm; $\bar{x}$ ± SE forceps length = `r myround(summary.table.2[[1,2]],2)` ± `r myround(summary.table.2[[1,5]],3)` mm, n = `r round(summary.table.2[[1,4]],0)`). Males formed two discrete clusters corresponding to brachylabic ($\bar{x}$ ± SE pronotum length = `r myround(summary.table.2[[3,6]],2)` ± `r myround(summary.table.2[[3,8]],3)` mm; $\bar{x}$ ± SE forceps length = `r myround(summary.table.2[[3,2]],3)` ± `r myround(summary.table.2[[3,5]],3)` mm, n = `r round(summary.table.2[[3,4]],0)`) and macrolabic morphs ($\bar{x}$ ± SE pronotum length = `r myround(summary.table.2[[2,6]],2)` ± `r myround(summary.table.2[[2,8]],3)` mm; $\bar{x}$ ± SE forceps length = `r myround(summary.table.2[[2,2]],2)` ± `r round(summary.table.2[[2,5]],3)` mm, n = `r round(summary.table.2[[2,4]],0)`), with overlap in body size but clear separation in forceps length (@fig-two).
...
::: {#fig-two fig-cap="Static allometries of forceps length as a function of pronotum length across density treatments. Females (top row) and males (bottom row) are shown separately. In males, brachylabic (solid points) and macrolabic (open points) morphs are distinguished, with diet treatments indicated by shading. Males form two clusters corresponding to morphotype, with overlap in body size between morphs. The dashed horizontal line in the male panels indicates the forceps length threshold used for morph classification."}
![](figure_2.jpg)
:::
